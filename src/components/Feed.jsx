...
const Feed = () => {
  ...
  // Ref so the effect closure always sees the latest feed without re-running
  const feedRef = useRef(feed);
  useEffect(() => { feedRef.current = feed; }, [feed]);

  useEffect(() => {
    // Feed already populated from a previous visit — nothing to do.
  ...
  // Derived: show spinner only while a fetch is in-flight and feed is empty
  const loading = !hasFetched && (!feed || feed.length === 0);

  const summary = (() => {
    if (loading) return "Loading developers…";
    if (!feed || feed.length === 0) return "No profiles available yet.";
    return `Showing ${feed.length} developer${feed.length > 1 ? "s" : ""} ready to connect.`;
  })();
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-slate-400">
                Explore matching developer profiles, send interest requests, and
                grow your collaboration network.
              </p>
            </div>
